...
public struct GroupedImportsRule: CorrectableRule, ConfigurationProviderRule, OptInRule, AutomaticTestableRule {
    ...
    private func violatingOffsets(inGroups groups: [[Line]], file: SwiftLintFile) -> [Int] {
        let allowedImportGroups = configuration.moduleGroups
        ...
        return groups.reduce(into: []) { partialResult, lines in
            ...
            let resultingImportGroups = allowedImportGroups.filter { group in
    ...
    public func correct(file: SwiftLintFile) -> [Correction] {
        ...
        groups.reversed().forEach { lines in
            ...
            // Требуемые группы импортов
            let resultingImportGroups = allowedImportGroups.compactMap { group in
                group.modules.intersection(declaredModules).isNotEmpty ? group : nil
            }
...
//    private func resultingLineGroups(inGroup: [Line]) -> [[Line]] {
//        
//    }
}
